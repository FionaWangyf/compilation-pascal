...
NumberStmt * new_number_stmt(char char_val);// 创建表示字符的NumberStmt
NumberStmt * new_number_stmt(double real_val);// 创建表示浮点数的NumberStmt
NumberStmt * new_number_stmt(long long int_val);// 创建表示整数的NumberStmt

//填充函数
//用特定类型的值填充已创建的NumberStmt对象。每个函数接受一个NumberStmt对象（通过指针或智能指针）和一个特定类型的值，然后设置对象的字段。
void fill_number_stmt(std::unique_ptr<NumberStmt> &num_value, long long int_val){
    num_value->is_signed = true;//设置为表示一个有符号整数
...
void fill_number_stmt(std::unique_ptr<NumberStmt> &num_value, double real_val){
    ...
    num_value->is_real = true;//设置为表示一个浮点数
    num_value->is_char = false;
    num_value->is_unsigned = false;
...
void fill_number_stmt(std::unique_ptr<NumberStmt> &num_value, char char_val){
    num_value->is_signed = true;
    num_value->is_real = false;
    num_value->is_char = true;//设置为表示一个字符
    num_value->is_unsigned = false;
...
enum class CurrentRule {
    ...
    ProgramHead,       // 程序头部分，包含程序名和可能的文件参数（例如：program Example(input, output);）
    ProgramBody,       // 程序主体，包含常量定义、变量声明、函数/过程声明和主程序语句
    IdList,            // 标识符列表，用于多个变量或参数的声明（例如：a, b, c）
    ConstDeclarations, // 常量声明部分（例如：const PI = 3.14;）
    ConstDeclaration,  // 单个常量的声明（例如：MAX_SIZE = 100;）
    ConstValue,        // 常量值（可以是数字、字符、字符串等）
    VarDeclarations,   // 变量声明部分（例如：var x, y: integer;）
    VarDeclaration,    // 单个或一组变量的声明（例如：count, sum: integer;）
    Type,              // 类型定义（例如：integer, real, array[1..10] of integer）
    BasicType,         // 基本类型（例如：integer, real, char, boolean）
    PeriodList,        // 数组维度范围列表（例如：1..10, 0..255）
    SubprogramDeclarations, // 子程序（函数和过程）声明部分
    Subprogram,        // 单个子程序（函数或过程）的声明
    SubprogramHead,    // 子程序头部，包含名称、参数和返回类型（对于函数）
    FormalParameter,   // 形式参数部分（例如：(x, y: integer; var z: real)）
    ParameterList,     // 参数列表（例如：x, y: integer; var z: real）
    Parameter,         // 单个参数或参数组（例如：x, y: integer）
    VarParameter,      // 引用传递参数（例如：var z: real）
    ValueParameter,    // 值传递参数（例如：x, y: integer）
    SubprogramBody,    // 子程序体，包含局部声明和语句
    CompoundStatement, // 复合语句块（例如：begin ... end）
    StatementList,     // 语句列表，多个语句用分号分隔
    Statement,         // 单个语句（赋值、条件、循环等）
    ProcedureCall,     // 过程调用（例如：WriteLn('Hello');）
    VariableList,      // 变量列表，用于输入/输出语句（例如：Read(a, b, c)）
    Variable,          // 变量引用，可以是简单变量或数组元素（例如：a, arr[i]）
    IdVarpart,         // 变量引用的附加部分，如数组下标（例如：[i, j]）
    ExpressionList,    // 表达式列表，用于函数调用或数组索引（例如：f(a, b+c, 5)）
    ArrayIndexExpression, // 数组索引表达式（例如：arr[i+1, j*2]）
    BracketExpressionList, // 方括号表达式列表，用于多维数组（例如：[i][j]）
    Expression,        // 完整表达式，可能包含关系运算（例如：a < b and c > d）
    SimpleExpression,  // 简单表达式，包含加法级运算符（例如：a + b - c）
    Term,              // 项，包含乘法级运算符（例如：a * b / c）
    Factor,            // 因子，表达式的基本构建块（例如：常量、变量、函数调用、括号表达式）
};
